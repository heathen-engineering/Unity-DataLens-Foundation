...
namespace Heathen.DataLens.Editor
{
    ...
    // DataLens Editor assembly is (deliberately, via asmdef defineConstraints) excluded from compilation.
    // Mirrors the equivalent bootstrap in Foundation for Steamworks.
    public static class DataLensMenuItems
    {
        ...
        [MenuItem("Help/Heathen/DataLens Foundation/Install Game Framework", priority = 1)]
        ...
        private static IEnumerator ValidateInstall()
        {
            ...
            {
                Debug.LogError("[DataLensMenuItems] Failed to query Package Manager: " + listReq.Error?.message);
                yield break;
            }

#if !HEATHEN_GAMEFRAMEWORK
            bool install = EditorUtility.DisplayDialog(
                "Heathen — DataLens Foundation",
        ...
        {
            if (SessionState.GetBool("DataLensGameFrameworkInstalling", false))
            {
                Debug.LogWarning("[DataLensMenuItems] An install is already in progress.");
                yield break;
            }

            SessionState.SetBool("DataLensGameFrameworkInstalling", true);

            AddRequest req = Client.Add(uri);
            Debug.Log($"[DataLensMenuItems] Installing package from {uri} …");

            while (!req.IsCompleted) yield return null;

            if (req.Status == StatusCode.Success)
                Debug.Log($"[DataLensMenuItems] {req.Result.displayName} {req.Result.version} installed successfully.");
            else
                Debug.LogError($"[DataLensMenuItems] Package install failed: {req.Error?.message}");

            SessionState.SetBool("DataLensGameFrameworkInstalling", false);
        }
